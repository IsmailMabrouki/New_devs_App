...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId = 'prop-001', debugTenant, showRaw }) => {
    ...
    // Use the string directly from the API for maximum precision
    // If it's a number, convert to string with 2 decimal places fixed
    const displayTotalStr = typeof data.total_revenue === 'string'
        ? parseFloat(data.total_revenue).toFixed(2)
        : data.total_revenue.toFixed(2);

    // For the locale formatting, we still need a number, but we format it correctly
    const displayTotalNum = parseFloat(displayTotalStr);
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300">
            ...
            <div className="p-6">
                    ...
                    <div>
                        ...
                        <div className="flex items-baseline gap-2 mt-1">
                            <span className="text-3xl font-bold text-gray-900 tracking-tight">
                                {data.currency} {displayTotalNum.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </span>
                ...
                <div className="mt-4 h-6">
                    {/* We compare the raw string value version vs. a converted float to check for potential frontend issues */}
                    {typeof data.total_revenue === 'string' && Math.abs(parseFloat(data.total_revenue) - displayTotalNum) > 0.000001 && showRaw && (
